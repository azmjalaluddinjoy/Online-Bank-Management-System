...
int main ()
{
    int option=1, choice, sign_in_option=1, sign_in_choice, i=0, j=0, k=0, l=0, m=0,n=0,p=0, sign_in_account_number,sign_in_password, signin_money_transfer_account, signin_change_pass_request_previous_password, signin_new_password_step1, signin_new_password_step2;
    ...
    inputfile = fopen("accounts_information.txt", "r");
        if (inputfile==NULL)
            {
                fprintf(stderr, "\n\"accounts_information.txt\"File isn't created yet !\nAfter Completing your all activity, a file will be created & the data will be saved for further use.\nPlease continue.\nThank You.\n");
            }
    ...
    while (option)
    {
        ...
        {
            FILE*outputfile;
            outputfile = fopen("accounts_information.txt", "a+");
                if (outputfile==NULL)
                    {
                        fprintf(stderr, "\nerror opening accounts_information.txt\n\n");
                        exit(1);
                    }
